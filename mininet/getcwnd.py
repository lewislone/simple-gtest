...
if __name__ == '__main__':
    ...
    for stream in streams:
        ...
        bw = [int(bw)/1024.0/1024 for bw in stream['bw']]
        pacing_rate = [int(pacing_rate)/1024.0/1024 for pacing_rate in stream['pacing_rate']]
        delivery_rate = [int(delivery_rate)/1024.0/1024 for delivery_rate in stream['delivery_rate']]
        reordering = [int(reordering) for reordering in stream['reordering']]

        print stream['local'], stream['peer']
        ##cwnd 
        grid(True)
        ylabel('pacing_rate', fontdict={'fontsize':20})
        ...
        legend(title=log_file + "(pacing_rate)", loc='best')
