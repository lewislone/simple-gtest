...
if [[ $tests == *"coexist"* ]]; then
    ...
    for cc_combo in cubic:1,oldbbr:1 cubic:1,bbr2:1; do
...
if [[ $tests == *"random_loss"* ]]; then
	...
	for cc_name in cubic bbr2 oldbbr; do
...
if [[ $tests == *"bufferbloat"* ]]; then
    ...
    # (bw=50M, rtt=30ms, buf={...}xBDP)
    for cc_name in cubic bbr2 odlbbr; do
...
if [[ $tests == *"ecn_bulk"* ]]; then
    ...
    enable_bbr_ecn
    for rep in `seq 1 10`; do
	for cc_name in dctcp bbr2 oldbbr; do
